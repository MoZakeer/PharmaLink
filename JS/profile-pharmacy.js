...
async function getUsers() {
    const response = await fetch(`https://pharmalink.runasp.net/api/profile/${userName}`, {
        method: 'GET',
    });
    ...
    document.querySelector('#address').innerHTML = `${data.city} / ${data.state} / ${data.street}`;
    document.querySelector(".about-content").innerHTML = data.aboutUs;
    let img;
    if (data['pharmacyImagePath']) {
        img = "https://pharmalink.runasp.net/" + data['pharmacyImagePath'].slice(data['pharmacyImagePath'].indexOf('uploads'));
